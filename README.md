...
### 核心功能（已实现）
- 自动截屏与会话聚合（默认 1 FPS，可配置）
- 视频生成与回放
- AI 分析与时间线：支持 Qwen、Claude、Codex CLI
- 活动标签与分类（基于分析结果可编辑）
- 本地存储与自动清理（SQLite/MariaDB）
- Notion 同步（可选）
- Obsidian 导出（每日/会话/周度索引/周报/总览 Markdown，含可配置的专注度与生产力评分）
- Obsidian 快捷入口（周报/索引/总览直达 + 周报评分卡）
- 配置迁移（导出/导入）
- 便携模式（可指定配置路径/指针，换机快速恢复）
- 多屏支持、黑屏检测、可配置分辨率
- 系统监控与日志查看
- 主界面月度概览（会话数、总时长、主要类别）

### 路线功能（规划中）
- Notion 视频上传与大文件处理策略

### 历史功能描述（待校准）
以下清单来自早期版本，可能与当前实现不一致，以“核心功能（已实现）/路线功能（规划中）”为准。
...
> 💡 **提示**：如果您只是想使用应用，请直接下载 [Releases](https://github.com/Nita121388/screen-analyzer/releases) 中的安装包，已包含所有必需文件。
...
前往 [Releases](https://github.com/Nita121388/screen-analyzer/releases) 页面下载对应平台的安装包：
...
#### 1. 克隆项目
```bash
git clone https://github.com/Nita121388/screen-analyzer.git
cd screen-analyzer
```
...
2. **配置 LLM（AI 设置）**
   ...
   - **通义千问 (Qwen)**：需要 API Key，可调整模型与 Base URL；配置后点击“测试连接”验证。
   - **Claude**：可填写 Auth Token/Base URL，留空则读取系统环境变量 `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL`；模型支持手动输入。
   - **Codex CLI**：需要本机安装并登录 `codex`，可设置 CLI 路径、模型、sandbox/审批策略；配置后点击“测试连接”验证。
...
4. **配置 Obsidian 导出**（可选）
   - 在应用设置中启用 “Obsidian 导出”
   - 填写 Vault 路径与根目录名
   - 选择导出模式（链接/复制）与是否包含截图、视频链接
   - 可配置周报评分权重与周目标时长
   - 使用“导出今日”或顶栏“Export”触发导出
...
### 文档与路线
- `技术路线说明.md`: 当前架构对齐与阶段路线
- `Obsidian集成方案.md`: Obsidian 导出策略与模板
- `配置迁移方案.md`: 配置导出/导入与换机流程
- `开发记录.md`: 阶段性开发记录与验证清单
...
### Q: 如何使用 Claude？
A:
1. 在应用设置中选择 "Claude" 提供商
2. 选择官方模型或手动输入兼容模型名称
3. 认证方式二选一：
   - Auth Token 留空：读取系统环境变量/Claude CLI 会话
   - 填写 Auth Token：优先使用该配置
4. Base URL 可选，用于代理或私有部署
5. 点击“测试连接”验证配置

**提示**：如使用兼容 Claude Agent 的模型，请确保接口与参数保持兼容，并自行验证效果。
...
- **视频生成**: FFmpeg (内置二进制)
- **LLM 集成**:
  - **通义千问 (Qwen)**: 走 OpenAI 兼容接口的多模态视觉模型
  - **Claude**: claude-agent-sdk 调用，支持 CLI 会话或 Auth Token
  - **Codex CLI**: 本地 `codex exec` 无头调用
...
- 项目主页: [https://github.com/Nita121388/screen-analyzer](https://github.com/Nita121388/screen-analyzer)
- Issue 反馈: [https://github.com/Nita121388/screen-analyzer/issues](https://github.com/Nita121388/screen-analyzer/issues)
